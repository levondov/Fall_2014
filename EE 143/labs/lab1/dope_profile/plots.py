...
profile6 = np.genfromtxt('profile6.txt', delimiter='    ')

profile6x_reduced = np.array([])
profile6y_reduced = np.array([])

# Get specific points so graph looks decent
for i in range(0,np.size(profile6[:,0])):
	if(profile5[i,0] <= 11):
		profile6x_reduced = np.append(profile6x_reduced,[profile6[i,0]])
		profile6y_reduced = np.append(profile6y_reduced,[profile6[i,1]])

plt.vlines(-0.1112,2,20,color='red',linestyles='dashed')
plt.vlines(0.2939,2,20,color='red',linestyles='dashed')
plt.hlines(21.4,-2,0.09628,color='green',linestyles='dashed')
plt.plot(profile6x_reduced,profile6y_reduced)
plt.scatter(profile6x_reduced,profile6y_reduced)
plt.axvspan(-0.1112, 0.2939, alpha=0.5, color='green')
plt.axvspan(0.2939, 2, alpha=0.5, color='red')
...
plt.xlabel("Distance ($\mu$m)")
plt.ylabel("log10(active(Boron))")
plt.title("Dopant profile after Sintering")
plt.grid(True)

plt.savefig('profile6.pdf')
