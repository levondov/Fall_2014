...
\subsection{Thin oxide MOSFET Cross Section}
...
\caption{W2: Field Oxidation. Field oxide thickness is 500 nm.}
\end{figure}
\begin{figure}[H]
\centering
...
\caption{W4: Gate Oxidation. Gate oxide thickness is about 80 nm.}
\end{figure}
\begin{figure}[H]
\centering
...
\caption{W5b: Polysilicon CVD. The poly thickness is about 350 nm.}
\end{figure}
\begin{figure}[H]
\centering
...
\caption{W7b: Source/Drain Drive-in + Intermediate Oxidation. The intermediate oxide thickness is 385 nm.}
\end{figure}
\begin{figure}[H]
\centering
...
\caption{W9: Aluminum Evaporation. The aluminum thickness is suppose to be about 800 nm}
\end{figure}
\begin{figure}[H]
\centering
...
\subsection{Comb-drive MEMS Structure Cross Section}
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=225pt]{calcs/vernier1.pdf}};
...
\textbf{Week 11: MEMS Structure Release} \\
Individual Processing Problems
\begin{itemize}
\item Failure to fully clear away sacrificial oxide will result in non-viable MEMS devices.
	\begin{itemize}
	\item Overetched for process latitude to account for thickness variation
	\end{itemize}
\item Due to relative thinness of the polysilicon layer used in this lab, MEMS devices are liable to collapse upon release, particularly due to surface tension if a wet etchant is used. 
	\begin{itemize}
	\item Use of a dry etchant to remove sacrificial oxide. (Not done in lab).
	\end{itemize}
\end{itemize}
Measurements
\begin{itemize}
\item No measurements taken. (MEMS release presumed to be unsuccessful in lab).
\end{itemize}
...
\centering
\begin{tabular}{c || c | c }
Step & \specialcell{Vertical \\ junction Depth \\ (nm)} & \specialcell{Lateral \\ junction Depth \\ (nm)} \\ \hline

Pre-diffusion & 365 & 321\\ \hline
Drive-in & 1000 & 880 \\ \hline
\end{tabular}
\caption{Junction depth measurements. Both vertically and laterally for pre-deposition and drive-in. Refer to Appendix \textcolor{blue}{\ref{sec:jdepth}} for the calculations.}
\label{fig:jdepth}
\end{figure}
...
Refer to Figure \textcolor{blue}{\ref{fig:jdepth}} for vertical and lateral junction depths. For details on the junction depth calculations see Appendix \textcolor{blue}{\ref{sec:jdepth}}. Also the reason we use a box approximation for the pre-diffusion is because we have a constant source profile. However, for drive-in we now have source-limited diffusion.
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile0.pdf}};
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile1.pdf}};
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile2.pdf}};
\node at (5.1,2.5) {\textbf{Si$\text{O}_2$}};
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile3.pdf}};
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile4.pdf}};
\node at (7,2.5) {\textbf{Si}};
...
\centering
\begin{tikzpicture}
\node at (5,5) {\includegraphics[width=350pt]{dope_profile/profile5.pdf}};
\node at (7,2.5) {\textbf{Si}};
